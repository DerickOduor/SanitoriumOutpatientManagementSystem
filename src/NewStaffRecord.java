...
@WebServlet("/NewStaffRecord")
public class NewStaffRecord extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if(visit_type.equalsIgnoreCase("First visit")&&patientType.equalsIgnoreCase("n_stf")){
                Date now=new Date();
                DateFormat dateFormat=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

                String qry="INSERT INTO patients(regno,name,dob,phone,relative,relative_phone,residence,home_address,d_o_a," +
                        "faculty,department,course,patientType,underAge,staffId,natId)VALUES('-','"+name+"','"+d_o_b+"','"+phone+"'," +
                        "'"+relative+"','"+relative_phone+"','-','"+home_address+"','"+dateFormat.format(now)+"','-','-'," +
                        "'-','Staff','False','"+staffId+"','"+natId+"')";
                ...
                String qry2="INSERT INTO patients_history(name,regno,complains,prescription,additional_info,blood_pressure," +
                        ...
                        "'"+0+"','"+dateFormat.format(now)+"','"+natId+"','Staff','False','"+staffId+"')";
